...
def get_train_test_list(datasplit):
        ...
        train_list = [item for item in all_list if item not in test_list and item not in ignor_list]
...
class FineDance_Smpl(data.Dataset):
    def __init__(self, args, istrain):
        self.motion_dir = r"C:\Users\vaisakh\AI Choreographer\data\finedance\motion"
        self.music_dir = r"C:\Users\vaisakh\AI Choreographer\data\finedance\music_npy"
        ...
        for name in tqdm(self.datalist):
            ...
            motion_path = os.path.join(self.motion_dir, name)
            music_path = os.path.join(self.music_dir, name)

            if not os.path.exists(motion_path) or not os.path.exists(music_path):
            ...
            if motion.shape[-1] == 168:
                ...
                raise ValueError("input motion shape error! not 168 or 319!")
